...
| シート名 | 内容 | 必須 |
|---|---|---|
| `入力_プラン設定` | 基本設定（生年月日・居住地・前提条件・シミュレーション終了年齢・年金条件・目標資産等） | ✓ |
...
FALSE（既定）の行は経常支出（`年間金額`・`成長率`・`柔軟支出フラグ`を使用、`入力_収入`と同じ
`年間金額`列名を共有）、TRUEの行は単発支出（`単発金額`と`開始条件タイプ`/`開始条件値`を使用、
`today`/`plan_start`/`age`/`date`のいずれかで発生年月を指定）として扱う。使われない側の列は
条件付き書式でグレーアウトされる。`入力_教育費`は`生年月日`列を追加しており、同じ子供IDの行
すべてに同じ生年月日を繰り返し記入する（行間で値が食い違うとエラーになる）。
...
- 必須セルには薄い黄色、任意入力のセルには薄い青の背景色を付け、入力欄かどうかと必須/任意を
  ...
  漏れない
- 口座タイプ・名義・資産クラス・開始/終了条件タイプ・居住都道府県・年金受給タイミングなど、
  選択肢が決まっている列にプルダウン（データの入力規則、`strict: true`）を設定する
- 単発フラグ・柔軟支出フラグ（TRUE/FALSE）はチェックボックスにする。ただしBOOLEAN型の
...
- `入力_支出`は単発フラグの値に応じて使われない列を条件付き書式でグレーアウトする
  （FALSEの行は単発金額・開始条件タイプ/値、TRUEの行は年間金額・成長率・柔軟支出フラグ）
- 期待リターン・年金受給開始年齢等の数値列は、文字列として保存されている場合は数値として
  書き込み直し、右揃い表示にする（`NUMERIC_HEADERS`。開始/終了条件値はage(数値)とdate(文字列)の
  どちらもあり得るため対象外）
- `入力_収入`の終了条件タイプ/値の列ヘッダーには、給与収入等の終了タイミングをここで設定する
  旨を説明するメモ（`HEADER_NOTES`）を付ける
...
条件を満たさず実行時に無視される入力値（例: 単発フラグ=FALSEの行に開始条件タイプが入力されている）
は、実行を止めずに`出力_エラー`シートへ「警告」として一覧表示する
...
- Monte Carlo/Historical Engineの資産クラス別加重合成は、AllocationPolicy設定時はその時点の
  ...
  反映されない。
- 収入（`Income`の`start_condition`/`end_condition`）は月精度で開始/終了を判定し、条件が年の
  途中で成立する年は該当月数分だけ按分計上する（`_active_months_in_year()`）。経常支出（`入力_支出`の
  単発フラグ=FALSEの行）には開始/終了条件の概念自体がなく、プラン全期間を通じて発生するものとして
  扱う。単発支出（単発フラグ=TRUEの行）は`resolve_condition_month()`により発生月を正確に判定する
  （Sprint14）。
...
- 所得税・住民税・社会保険料は年1回の確定計算のみで、その結果を12等分して毎月のキャッシュフローに
  反映する（月ごとの累進課税の再計算はしない。日本の税制がそもそも年次確定であるため）。公的年金等控除は
...
- 決定論的シミュレーション（`run_projection`）は口座ごとの期待リターン（`入力_口座`の期待リターン）で
  ...
  口座ごとの期待リターン・ボラティリティは使わない（相関構造を重視するため）。
