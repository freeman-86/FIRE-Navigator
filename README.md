...
- **教育費・大型一時支出**（Sprint14）: 子供の年齢帯に応じた教育費（`入力_教育費`、任意）を毎月の
  支出に自動加算する。車・旅行・住宅購入等の単発支出（`入力_大型支出`、任意）は指定した年月
  （today/plan_start/age/date）にのみ全額を一括計上する（月次化により発生月をそのまま扱える）
...
| `入力_子供` | 子供の一覧（子供ID・生年月日） | 任意 |
| `入力_教育費` | 子供の年齢帯別の教育費（小学校・塾・中学・高校・大学等、月額） | 任意 |
| `入力_大型支出` | 車・旅行・住宅購入等の単発支出（発生年月を指定） | 任意 |
...
- 収入・支出の開始/終了条件（`Income`/`Expense`の`start_condition`/`end_condition`）は現時点では
  ...
  収支の発生タイミング自体の月精度化は今後の課題）。一方、`入力_大型支出`（単発支出）は
  `resolve_condition_month()`により発生月を正確に判定する（Sprint14）。
- 教育費（`入力_教育費`）・大型一時支出（`入力_大型支出`）には`inflation_rate`と同様に物価上昇率は
  適用されない（名目固定額のまま）。教育費バンドは年齢帯（開始年齢〜終了年齢を含む）ベースで判定し、
  子供の誕生月は考慮しない（`年 - 生年.year`という簡易な年齢計算。既存のuser年齢計算と同じ簡略化）。
