...
- **教育費・単発支出**（Sprint14）: 子供の年齢帯に応じた教育費（`education_expenses`、任意）を
  ...
  一括計上する）
- マイルストーン到達判定・感応度分析（支出水準増減×インフレ率）
...
開始/終了条件をずらした複数行に分けて表現する）、`one_time`の行は単発支出（単発金額・成長率と
開始条件タイプ/値で発生年月を指定する。単発支出には終了条件の概念はない）として扱う。両方の
`kind`とも成長率は任意で、未入力の行は`assumptions.inflation_rate`を既定値として使う
（`recurring`は毎月の発生額に、`one_time`は発生年までの複利計算に用いる。いずれも
「プラン開始時点の価値」で入力した金額を、発生時点までその行の成長率で複利計算してから
計上する）。`recurring`と`one_time`で互いのフィールドを混在させることは許されず
（局所的な警告ではなく即エラーで拒否する）、教育費（`education_expenses`）は同じ子供IDの行
すべてに同じ生年月日を`children`側で一度だけ記入する。
...
- `inflation_rate`（`assumptions`）は、収入/支出で成長率が未入力の行の既定値として
  ...
  local_data_adapter._parse_growth_rate`）に行われ、読み込み後のIncome/Expenseは元の
  inflation_rateと同じ値のRateを持つだけで「未入力だった」という情報自体は失われる。感応度分析で
  inflation_rateを振る際は、growth_rateが元のinflation_rateと一致する行を「未入力だった行」とみなし
  新しいinflation_rateへ付け替えることで対応している（`sensitivity_analysis._apply_inflation_delta`）。
  成長率を明示的に元のinflation_rateと同じ値へ設定していた行との区別はできないため、あくまで近似。
...
- 収入（`Income`の`start_condition`/`end_condition`）・経常支出（`Expense`の`start_condition`/
  ...
  該当月数分だけ按分計上する（`_active_months_in_year()`）。年間合計だけでなく月次詳細（出力_月次詳細
  相当）の各月の金額自体も、その月ちょうどの有効/無効を個別判定して算出する
  （`_income_amount_for_month()`/`_recurring_expense_amount_for_month()`。年金収入も同様に
  `_pension_amount_for_month()`で判定する）。年間合計をブロックの月数で均等按分するだけだと、
  本来収入・支出が発生していないはずの月にも金額が計上されてしまうため（税額の月割りとは違い、
  収入・支出そのものの金額を月ごとにならす経済的な理由がないため区別している）。経常支出は両方省略すればこれまで通り
...
- 所得税・住民税・社会保険料は年1回の確定計算のみで、その結果をその区切りの実際の月数で割って
  ...
  対応しない（MVP簡略化）。
- 住民税は「前年の所得」に基づき課税される実際の制度を反映している（`core/simulation/tax/
  tax_engine.py`の`calculate_tax()`の`prior_year_employment_income`/`prior_year_pension_income`）。
  そのため退職・FIREした年（当年の収入がほぼ0円になる年）も、前年の所得に基づいた住民税が
  そのままキャッシュフローに反映される（早期退職者が想定外に資金繰りを圧迫されがちな、日本
  特有の挙動）。シミュレーション最初の年は「前年」の実績がないため、当年の所得をそのまま
  前年所得として使う近似。年齢区分（65歳以上判定）・配偶者控除の有無・iDeCo拠出額等の控除前提は
  当年と同じ値を前年分の計算にも流用する近似（年ごとに個別追跡するコストに対して得られる精度
  向上が小さいため）。所得税・社会保険料は制度通り当年の所得に基づく。
...
- 決定論的シミュレーション（`run_projection`）は口座ごとの期待リターン（`accounts`の期待リターン、
  ...
  成長率はいずれもゼロ成長として扱う）。Monte Carlo/Historical Engineは従来通り、過去実績データの
  資産クラス別分布を`allocation_policy`の資産クラス比率で加重合成した単一レートを全口座に一律適用する
  設計のままで、口座ごとの期待リターンは使わない（相関構造を重視するため）。
- 感応度分析（`sensitivity_analysis.run_sensitivity_analysis`）は支出水準×インフレ率のグリッド
  （行=支出水準増減、列=インフレ率増減）。以前は「口座期待リターン±1%」という軸を持っていたが、
  同じダッシュボードのMonte Carlo/Historicalの方がリターンの不確実性を分布として遥かに高精度に
  表現できており、決定論的な3点評価は二重表現で情報価値が低いため廃止した。代わりに
  Monte Carloでは表現できない「ユーザー自身が選べる意思決定」の軸として支出水準
  （経常支出・単発支出・教育費すべてに一律の増減幅、収入には影響しない）を採用している。
